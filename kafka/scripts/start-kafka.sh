...
if [ ! -z "$ADVERTISED_HOST" ]; then
    ...
    if grep advertised.host.name $KAFKA_HOME/config/server.properties; then
...
if [ ! -z "$ADVERTISED_PORT" ]; then
    ...
    if grep advertised.port $KAFKA_HOME/config/server.properties; then
...
do
  ...
  if [[ $VAR =~ ^KAFKA_ && ! $VAR =~ ^KAFKA_HOME ]]; then
    ...
    echo assign "$kafka_name=$env_var"
  fi
done
